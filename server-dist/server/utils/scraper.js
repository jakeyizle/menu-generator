            ...
            try {
                ...
                for (const jsonLd of jsonLdObjects) {
                    ...
                    if (jsonLd['@type'] === 'Recipe' ||
                        (jsonLd['@graph'] && jsonLd['@graph'].some((item) => item['@type'] === 'Recipe'))) {
                        // Get the recipe object
                        const recipeObj = jsonLd['@type'] === 'Recipe' ?
                            jsonLd :
                            jsonLd['@graph'].find((item) => item['@type'] === 'Recipe');
                        if (recipeObj) {
 ...
 */
function mapJsonLdToRecipe(jsonLd) {
    // Extract ingredients
    const ingredients = [];
    if (Array.isArray(jsonLd.recipeIngredient)) {
        jsonLd.recipeIngredient.forEach((ingredientStr) => {
 ...
 */
function determineMealTypes(name, description) {
